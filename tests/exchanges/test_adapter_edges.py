...
async def test_testnet_calls_set_sandbox_mode(
    ...
    client = patch_client()
    adapter = await ExchangeAdapter.create("bybit", testnet=True)
    async with adapter:
...
async def test_no_testnet_does_not_touch_sandbox(
    ...
    client = patch_client()
    adapter = await ExchangeAdapter.create("bybit", testnet=False)
    async with adapter:
...
async def test_market_missing_symbol_is_dropped(
    ...
    async with adapter:
        ...
        assert [m.symbol for m in markets] == ["BTC/USDT"]
...
async def test_order_book_skips_level_with_non_numeric_price(
    ...
    adapter = await ExchangeAdapter.create("bybit")
    async with adapter:
        book = await adapter.fetch_order_book("BTC/USDT")
