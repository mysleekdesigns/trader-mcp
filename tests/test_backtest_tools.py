...
async def test_phase4_adds_no_execution_surface() -> None:
    """Backtest tools add no order/arming/credential surface (safe-by-default)."""
    names = {t.name for t in await build_app().list_tools()}
    assert names.isdisjoint({"place_order", "deploy_strategy", "arm_live_trading", "kill_switch"})
