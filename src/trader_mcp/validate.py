...
the PRD §6 Phase 1 exit item ("Deep-validate Bybit; smoke-test BloFin/Toobit/WeeX;
log known quirks"). This is the command you run from an allowed-region vantage point
(VPN, proxy, or unblocked CI runner) when this dev host is geo-blocked.
...
Exit code: ``0`` only if the Bybit deep-validation fully passes (so CI can gate on it).
Smoke-test failures on the non-reference exchanges are reported but do not fail the run
-- they are recorded as quirks.
"""
...
#: Default reference perp symbol for funding-rate + perp checks.
DEFAULT_PERP = "BTC/USDT:USDT"
#: Default spot symbol for ticker/ohlcv/order-book/trades checks.
DEFAULT_SPOT = "BTC/USDT"
...
class ExchangeReport:
    ...
    exchange: str
    mode: str  # "deep" (Bybit) or "smoke" (others)
...
async def _deep_validate(adapter: ExchangeAdapter, *, spot: str, perp: str) -> list[CheckResult]:
    """Exercise all 9 market-data paths and assert basic invariants (Bybit)."""
    ...
    async def funding_rate() -> CheckResult:
        ...
        if fr.funding_rate is None:
            return _fail("funding_rate", "funding_rate missing")
...
async def _validate(exchanges: list[ExchangeId], *, spot: str, perp: str) -> list[ExchangeReport]:
    """Run deep-validation on Bybit and a smoke test on the others."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    Returns a process exit code: ``0`` only when Bybit deep-validation fully passes.
    """
    ...
    # Gate on the reference exchange (Bybit). Others are smoke/quirks-only.
    bybit = next((r for r in reports if r.exchange == "bybit"), None)
    failed = sum(1 for r in reports for c in r.checks if not c.ok)
    print(f"\n{failed} check(s) failed across {len(reports)} exchange(s).")
    if bybit is not None and not bybit.passed:
        print("Bybit deep-validation FAILED.")
        return 1
    return 0
